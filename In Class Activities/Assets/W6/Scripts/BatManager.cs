...
public class BatManager : MonoBehaviour
{
    ...
    // STEP 1 -----------------------------------------------------------------
    [SerializeField] private BatW6[] _bat;
    ...
    private void Start()
    {
        ...
        //      and so on.
        _newTextTimers = new float[_bat.Length - 1];
        //subtract one from whatever length so that you dont have an extra element 
        // STEP 6 -------------------------------------------------------------
    }
    ...
    private void Update()
    {
        ...
        {
            float position = Vector3.Distance(_bat[i].transform.position, _playerTransform.position);
            if (position < _interactDistance)
            {
                _bat[i].EnableChase(_playerTransform);
            }
            ...
            if (position < _overlapDistance)
            {
                CreateReactions(_bat[i]);
            }
    ...
    private void CreateReactions(BatW6 bat)
    {
        ...
        string _message = _messages[randomIndex];
    ...
    private void SpawnReactionUI(BatW6 bat, string message)
    {
        ...
        int index = System.Array.IndexOf(_bat, bat);
